...
function updateSourcePreview(actor_id) {
    ...
    $.ajax({
            url: "/actors/"+actor_id+"/get_source_code",
            success: function(data){
                var codes = JSON.parse(data.data);
                $("#src_preview_textarea").val(codes.source_code);
                $("#sta_preview_textarea").val(codes.static_code);
            }
        });
}
