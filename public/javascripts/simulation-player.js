...
function load_story() {
    $.ajax({
            ...
            success: function(data){
                ...
                if (story.length == 1) {
                    alert("This simulation hasn't been run yet. Press simulate button and create some simulation steps.");
                }
...
function rewindSimulation() {
    ...
    if ((newStep >= 1) && (newStep < story.length-1)) {
...
$(document).ready(function() {
    ...
    $("#chk_autoplay").change(function() {
        if ($('#chk_autoplay').attr('checked') == true) {
            $(document).everyTime(500, 'autoplay', function(i) {
                    advanceSimulationUp();
            });
        } else {
            $(document).stopTime();
